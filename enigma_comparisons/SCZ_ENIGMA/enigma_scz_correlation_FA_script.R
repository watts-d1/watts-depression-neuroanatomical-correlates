# --------------------------------------------------------
# ENIGMA - Calculating correlations with SCZ working group summary statistics 
# --------------------------------------------------------
...
# Create the SCZ data frame with category information
SCZ_FA = data.frame(
  ROI = c("ACR", "CC", "BCC", "GCC", "ALIC", "CR", "FXST", "FX", "PTR", "SS",
          "SFO", "CGC", "PCR", "SCC", "SLF", "EC", "IC", "UNC", "SCR", "RLIC",
          "IFO", "CGH", "CST", "PLIC"),
  Full.tract.name = c("Anterior corona radiata", "Corpus callosum", 
                      "Body of corpus callosum", "Genu of corpus callosum",
                      "Anterior limb of internal capsule", "Corona radiata",
                      "Fornix/Stria terminalis", "Fornix",
                      "Posterior thalamic radiation", "Sagittal stratum",
                      "Superior fronto-occipital fasciculus", "Cingulum (cingulate)",
                      "Posterior corona radiata", "Splenium of corpus callosum",
                      "Superior longitudinal fasciculus", "External capsule",
                      "Internal capsule", "Uncinate fasciculus",
                      "Superior corona radiata", "Retrolenticular part of IC",
                      "Inferior fronto-occipital fasciculus", "Cingulum (hippocampal)",
                      "Corticospinal tract", "Posterior limb of internal capsule"),
  Cohens_D = c(-0.40, -0.40, -0.39, -0.37, -0.37, -0.33, -0.32, -0.31, -0.31, -0.30,
               -0.29, -0.27, -0.25, -0.22, -0.22, -0.21, -0.18, -0.16, -0.15, -0.13,
               -0.11, -0.11, -0.04, 0.04),
...
# load the Cohen's d results lists
...
###### Process white matter tract data ######
...
FA_processed = process_white_matter_data(SCZ_FA, "fa")
...
# Process lookup table to match ENIGMA SCZ naming conventions
matching_table_FA$Region = toupper(matching_table_FA$Region)

# Create mapping between ENIGMA SCZ and UKBB features
SCZ_FA$processed_name_abbreviated = paste0("fa_", tolower(SCZ_FA$ROI))

# Merge SCZ FA data with matching table
SCZ_FA_matched = merge(SCZ_FA, 
...
# use Cohen's d from the results list
...
results_list_FA_merged = lapply(results_list_FA, function(df) {
  ...
  merged_results = merge(averaged_results,
                         SCZ_FA[, c("ROI", "Cohens_D", "Category")],
                         by.x = "Region",
                         by.y = "ROI",
                         all.x = TRUE)
...
# --------------------------------------------------------
# Calculate correlations between SCZ ENIGMA and UKBB effect sizes
# --------------------------------------------------------

calculate_correlations = function(df, col1, col2) {
  # grab the two effect-size columns to correlate
  cor_data = data.frame(
    x = df[[col1]],
    y = df[[col2]]
  )
  ...
  # spearman p-value, NA if it fails
  spearman_p = tryCatch(
...
create_scatter_plot = function(data, definition, corr, p_value, global_range, color) {
  # plot UKB vs ENIGMA effect sizes with lm fit
  ggplot(data[[definition]], aes(x = cohen_d, y = d_enigma)) +
    ...
    xlab("Cohen's d (UKB)") +
    ylab("Cohen's d (ENIGMA SCZ)") +
    # pin axes to shared global ranges across plots
    ...
    scale_x_continuous(breaks = seq(min(global_range$x_range), max(global_range$x_range), length.out = 5),
                       ...
                       expand = c(0.01, 0)) +
    theme_classic() +
    # match all axis and title styling to the plot color
    theme(
