...
training_data_shallow_subset = training_data_shallow %>%
  select(all_of(names(training_features_deep)))

testing_data_shallow_subset = testing_data_shallow %>%
  select(all_of(names(testing_features_deep)))

classes_training_downsampled = table(training_data_shallow_subset$outcome)
classes_testing_downsampled = table(testing_data_shallow_subset$outcome)
...
task_train = TaskClassif$new("Outcome Train", 
                             backend = training_data_shallow_subset, 
                             target = "outcome", 
...
search_space_prediction = ps(# mtry: Number of variables randomly sampled as candidates at each split
  mtry = p_int(lower = 1, upper = floor(sqrt(ncol(training_data_shallow_subset)))),
...
f_controls = table(training_data_shallow_subset$outcome)[1]
f_cases    = table(training_data_shallow_subset$outcome)[2]
w_cases    = f_controls / (f_controls + f_cases)
w_controls = f_cases / (f_controls + f_cases)

weights = ifelse(training_data_shallow_subset$outcome == '1', w_cases, w_controls)

set.seed(150)
per_model = ranger::ranger(dependent.variable.name = "outcome",
                           data = training_data_shallow_subset,
...
predictions = predict(per_model, data = testing_data_shallow_subset, type = "response") 

# Performance metrics

confusion_matrix = table(testing_data_shallow_subset$outcome, predictions$predictions)
...
prediction_probabilities = predict(auc_model, data = testing_data_shallow_subset, type = "response")

roc_curve = roc(testing_data_shallow_subset$outcome, prediction_probabilities$predictions[,2])
...
predictions_auc = predict(auc_model, data = testing_data_shallow_subset, type = "response") 
predicted_probabilities_auc = predictions_auc$predictions[, 2]

roc_obj = roc(testing_data_shallow_subset$outcome, predicted_probabilities_auc)
...
true_labels = testing_data_shallow_subset$outcome
...
adjusted_confusion_matrix_sens = table(testing_data_shallow_subset$outcome, adjusted_predictions_sens)
...
for (i in 1:n_bootstraps) {
  bootstrap_indices = sample(1:length(testing_data_shallow_subset$outcome), replace = TRUE)
  bootstrap_true_labels = testing_data_shallow_subset$outcome[bootstrap_indices]
...
save(var_importance_df, var_importance_lookup, file = here("output", "feature_transfer", "var_importance_shallow_task_deep_control_features_feature_transfer.RData"))

save(binarized_predictions, true_labels_numeric, f1_score_sens, auc, roc_df, auc_model, roc_obj, ci_auc, ppv_sens, npv_sens, file = here("output", "feature_transfer", "shallow_task_deep_control_features_binary_classifier_significance.RData"))
