...
table(combined_shallow_phenotype_cleaned$outcome)
...
# Downsampling shallow cases to match deep model case-control ratio
# For the deep model we have:
#     control:       17,113
#     deep_depression: 5,712
#
# For the shallow model currently we have:
#     control:           17,112
#     shallow_depression: 11,424
#
# To match the deep model ratio (≈3:1), we want the shallow_depression cases to be 5,711.
# (Note: since 5,711 * 3 = 17,133 but only 17,112 controls are available, we sample the controls as is.)
#
set.seed(707)

# Downsample shallow_depression cases to 5,711 (from the available 11,424)
shallow_cases_downsampled = combined_shallow_phenotype_cleaned %>%
  filter(outcome == "shallow_depression") %>%
  sample_n(5711)

# (Optional) Re-sample controls to ensure exactly 17,112 are used.
# In your current data these already number 17,112, but we include this for symmetry.
controls_downsampled = combined_shallow_phenotype_cleaned %>%
  filter(outcome == "control") %>%
  sample_n(17112)

# Recombine the downsampled cases and controls
combined_shallow_phenotype_cleaned = bind_rows(
  controls_downsampled,
  shallow_cases_downsampled
) %>%
  arrange(subject_num)

print("Downsampled shallow phenotype counts:")
print(table(combined_shallow_phenotype_cleaned$outcome))

# Stratified sampling 
set.seed(505)  
data_split = initial_split(combined_shallow_phenotype_cleaned, prop = 0.7, strata = "outcome")
train_data_downsampled = training(data_split)
test_data_downsampled  = testing(data_split)
...
train_for_rfe = train_data_downsampled %>% select(-outcome)
outcome = as.factor(train_data_downsampled$outcome)

sizes_to_evaluate = c(76, 100, 150, 226)

results = rfe(train_for_rfe,
              train_data_downsampled$outcome,
...
training_rfe_downsampled = train_data_downsampled %>%
  select(all_of(top_features), outcome)

testing_rfe_downsampled = test_data_downsampled %>%
  select(all_of(top_features), outcome)
...
task_train = TaskClassif$new("Outcome Train", 
                             backend = training_rfe_downsampled, 
                             target = "outcome", 
...
search_space_prediction = ps(# mtry: Number of variables randomly sampled as candidates at each split
  mtry = p_int(lower = 1, upper = floor(sqrt(ncol(training_rfe_downsampled)))),
...
f_controls = table(training_rfe_downsampled$outcome)[1]
f_cases    = table(training_rfe_downsampled$outcome)[2]
w_cases    = f_controls / (f_controls + f_cases)
w_controls = f_cases / (f_controls + f_cases)

weights = ifelse(training_rfe_downsampled$outcome == '1', w_cases, w_controls)

set.seed(210)
per_model = ranger::ranger(dependent.variable.name = "outcome",
                           data = training_rfe_downsampled,
...
predictions = predict(per_model, data = testing_rfe_downsampled, type = "response") 

# Performance metrics

confusion_matrix = table(testing_rfe_downsampled$outcome, predictions$predictions)
...
prediction_probabilities = predict(auc_model, data = testing_rfe_downsampled, type = "response")

roc_curve = roc(testing_rfe_downsampled$outcome, prediction_probabilities$predictions[,2])
...
predictions_auc = predict(auc_model, data = testing_rfe_downsampled, type = "response") 
predicted_probabilities_auc = predictions_auc$predictions[, 2]

roc_obj = roc(testing_rfe_downsampled$outcome, predicted_probabilities_auc)
...
true_labels = testing_rfe_downsampled$outcome
...
adjusted_confusion_matrix_sens = table(testing_rfe_downsampled$outcome, adjusted_predictions_sens)
