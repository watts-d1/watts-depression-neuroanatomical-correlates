...
INPUT_DATA_PATH = "/path/to/non_overlapping_control_df.RData"  # Non-overlapping control dataframe from S1
LOOKUP_TABLE_PATH = "/path/to/lookup_table_neuroimaging_ukbb.xlsx"  # Neuroimaging variable lookup table
WORKSPACE_PATH = "/path/to/deep_vs_controls_residualized_removing_dup_control_deep_cases_top30.RData"  # Full workspace from initial model run
...
# Apply modify_outcomes function to shallow and deep outcomes
# Replacing 0 with "control" and 1 with the specified depression type
...
# Integrate modified outcomes into phenotypes
...
# Remove original outcome columns
...
# Remove covariate columns from combined phenotypes
...
# Prioritize deep_depression subjects by removing them from shallow
...
combined_shallow_phenotype_cleaned = combined_shallow_phenotype %>%
  filter(
    !(outcome == "shallow_depression" & subject_num %in% subjects_with_deep)
  )

combined_shallow_phenotype_cleaned$outcome = droplevels(as.factor(combined_shallow_phenotype_cleaned$outcome))

# Deduplicate controls across both datasets
shallow_controls = combined_shallow_phenotype_cleaned %>%
  filter(outcome == "control")

deep_cases = combined_deep_phenotype %>% filter(outcome == "deep_depression")
deep_controls = combined_deep_phenotype %>% filter(outcome == "control")

all_controls = bind_rows(shallow_controls, deep_controls) %>%
  distinct(subject_num, .keep_all = TRUE)

combined_deep_vs_control = bind_rows(deep_cases, all_controls) %>%
  arrange(subject_num)

combined_deep_vs_control$outcome = droplevels(combined_deep_vs_control$outcome)
combined_deep_vs_control$MDDRecur = NULL

# Verify no subjects have both deep_depression and control
subject_check = combined_deep_vs_control %>%
  group_by(subject_num) %>%
  summarise(n_outcomes = n_distinct(outcome)) %>%
  filter(n_outcomes > 1)

if (nrow(subject_check) > 0) {
  stop("Subjects with conflicting classifications exist!")
} else {
  message("Verification passed. Subjects are uniquely classified.")
}

message("Final outcome counts:")
table(combined_deep_vs_control$outcome)

# Stratified 70/30 train/test split
set.seed(404)
data_split = initial_split(combined_deep_vs_control, prop = 0.7, strata = "outcome")
train_data = training(data_split)
test_data  = testing(data_split)

train_ids = train_data %>% select(subject_num, outcome)
test_ids = test_data %>% select(subject_num, outcome)

save(train_ids,
     test_ids,
     file = here("output", "deep_vs_controls", "train_test_ids.RData"))
...
# Select the top 76 features (excluding the outcome variable)
...
# Inverse-frequency class weights to handle imbalanced classes
...
predictions = predict(per_model, data = test_data, type = "response")

# Performance metrics
confusion_matrix = table(test_data$outcome, predictions$predictions)

# Refit with probability = TRUE for AUC calculation
set.seed(160)
...
prediction_probabilities = predict(auc_model, data = test_data, type = "response")
roc_curve = roc(test_data$outcome, prediction_probabilities$predictions[,2])
...
predictions_auc = predict(auc_model, data = test_data, type = "response") 
predicted_probabilities_auc = predictions_auc$predictions[, 2]

roc_obj = roc(test_data$outcome, predicted_probabilities_auc)
...
adjusted_confusion_matrix_sens = table(test_data$outcome, adjusted_predictions_sens)
...
for (i in 1:n_bootstraps) {
  bootstrap_indices = sample(1:length(test_data$outcome), replace = TRUE)
...
# Print accuracy with 95% confidence interval
...
# Print sensitivity and specificity with 95% confidence intervals
...
# Print PPV and NPV with 95% confidence intervals
...
save(var_importance_df, var_importance_lookup, file = here("output", "deep_vs_controls", "var_importance_deep_controls_removing_dup_control_deep_cases_top30_percent.RData"))

save(binarized_predictions, true_labels_numeric, f1_score_sens, auc, roc_df, roc_obj, ci_auc, ppv_sens, npv_sens, file = here("output", "deep_vs_controls", "deepcontrols_removing_dup_control_deep_cases_metrics_top30_percent.RData"))

load(WORKSPACE_PATH)
load(here("output", "deep_vs_controls", "train_test_ids.RData"))

# Validate and add subject_num back to RFE datasets
if(nrow(training_rfe) != nrow(train_ids) || 
   any(training_rfe$outcome != train_ids$outcome)) {
  stop("inconsistent match between id object and train/test split, check data structure")
} 

training_rfe$subject_num = train_ids$subject_num

if(nrow(testing_rfe) != nrow(test_ids) || 
   any(testing_rfe$outcome != test_ids$outcome)) {
  stop("inconsistent match between id object and train/test split, check data structure")
}

testing_rfe$subject_num = test_ids$subject_num

save(training_rfe,
     testing_rfe,
     file = here("output", "deep_vs_controls", "training_testing_rfe_with_subject_num.RData"))
